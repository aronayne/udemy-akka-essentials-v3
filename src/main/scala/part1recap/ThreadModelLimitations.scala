...
object ThreadModelLimitations extends App{
  ...
  /**
   * D2 #1: OOP encapsulation is only valid in the SINGLE THREADED MODEL
   */
...
//  println(account.getAmount)
  ...
  val futures = (0 to 9) //10 futures
    .map(i => 100000 * i until 100000 * (i + 1)) // 0 - 99999, 100000 - 199999 , 200000 - 299999 etc
    .map(range => Future {
      if (range.contains(546753)) throw new RuntimeException("invalid number") //Finding this error in a prod environment is difficult
      range.sum
    })
